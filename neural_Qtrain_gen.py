...
def init(env, env_name):
    ...
    action spaces
    """
    global replay_buffer, epsilon, iscontinuous, continuous_action_mapping
    replay_buffer = []
    ...
    try:
        ...
        iscontinuous = True
        action_dim = CONTINUES_DIM
        continuous_action_mapping = np.linspace(env.action_space.low[0],env.action_space.high[0],CONTINUES_DIM)
...
def get_network(state_dim, action_dim, hidden_nodes=HIDDEN_NODES):
    ...
    c_names, n_l1, w_initializer, b_initializer = \
            ['eval_params', tf.GraphKeys.GLOBAL_VARIABLES], hidden_nodes, \
            tf.random_normal_initializer(0., 0.3), tf.constant_initializer(0.1)

    with tf.variable_scope('l1'):
        w1 = tf.get_variable('w1', [state_dim, n_l1], initializer=w_initializer, collections=c_names)
        b1 = tf.get_variable('b1', [1, n_l1], initializer=b_initializer, collections=c_names)
        l1 = tf.nn.relu(tf.matmul(state_in, w1) + b1)

    with tf.variable_scope('l2'):
        w2 = tf.get_variable('w2', [n_l1, action_dim], initializer=w_initializer, collections=c_names)
        b2 = tf.get_variable('b2', [1, action_dim], initializer=b_initializer, collections=c_names)
...
def get_env_action(action):
    ...
    """
    if iscontinuous:
        return [continuous_action_mapping[action]]
    return action
...
def update_replay_buffer(replay_buffer, state, action, reward, next_state, done,
    ...
    one_hot_encoded = [0]*action_dim
    one_hot_encoded[action] = 1
    # append to buffer
    replay_buffer.append([state, np.array(one_hot_encoded), reward, next_state, done])
